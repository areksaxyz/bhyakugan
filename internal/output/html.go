...
func GenerateHTML(filename string, rawFindings []core.Finding, liveHosts []string, target string) error {
	...
	for _, f := range rawFindings {
		if f.Severity == "Critical" || f.Severity == "High" {
			...
			if isNoSQL {
				if endpointTypes[ep] == "SQL" { continue } // Skip NoSQL if already confirmed SQL (rare)
				endpointTypes[ep] = "NoSQL"
				endpointImpacts[ep]["Authentication Logic Bypass (NoSQL)"] = true
			} else if isSQLi {
				if endpointTypes[ep] == "NoSQL" { continue } // Skip SQLi if already confirmed NoSQL
				endpointTypes[ep] = "SQL"
				endpointImpacts[ep]["Database Interaction Control (SQLi)"] = true
			} else {
				// Other types don't conflict
				if strings.Contains(f.Type, "LFI") || strings.Contains(f.Type, "Path Traversal") { endpointImpacts[ep]["Arbitrary File Read (LFI)"] = true }
				if strings.Contains(f.Type, "Secret Leak") { endpointImpacts[ep]["Credential Exposure (Secrets)"] = true }
				if strings.Contains(f.Type, "RCE") { endpointImpacts[ep]["Remote Code Execution (RCE)"] = true }
				if strings.Contains(f.Type, "SSRF") { endpointImpacts[ep]["Internal Infrastructure Access (SSRF)"] = true }
			}
		}
	}
...
`, target, target, target, time.Now().Format("Jan 02, 2006 15:04:05 MST"), len(liveHosts), stats["Critical"], stats["High"], stats["Medium"], stats["Low"], stats["Info"])
	...
	for _, sev := range severities {
        ...
        <div class="table-container">
            <table>
                <thead><tr><th class="col-type">Vulnerability Type</th><th class="col-target">Target Endpoint</th><th class="col-detail">Evidence / Details</th></tr></thead>
		...
		for _, fnd := range items {
            ...
            <tr>
                ...
                <td class="col-target"><a href="%s" target="_blank">%s</a></td>
                <td class="col-detail">%s</td>
            </tr>`, fnd.Type, fnd.Target, displayTarget, fnd.Detail)
...
func consolidateFindings(findings []core.Finding) []core.Finding {
	...
	for key, group := range grouped {
		...
		shouldConsolidate := len(group) > 1 && (
			strings.Contains(fType, "Web Cache") || 
