...
func Scan(baseURL string, client *http.Client, ctx core.ScanContext, onFound func(core.Finding)) {
	...
	if baselineDuration > 3.0 { return }
...
func checkTarget(target string, payload SQLiPayload, client *http.Client, baselineDuration, timeThreshold float64, baselineBodyLower string, isAlreadyVulnerable *bool, foundMu *sync.Mutex, onFound func(core.Finding)) {
	...
	if strings.Contains(payload.Name, "Cloudflare") { localThreshold = baselineDuration + 5.0 }
	...
	for db, errors := range DBErrors {
		for _, errStr := range errors {
			if strings.Contains(bodyLower, strings.ToLower(errStr)) {
				if strings.Contains(baselineBodyLower, strings.ToLower(errStr)) { continue }
				isVulnerable = true
	...
	if !isVulnerable && strings.Contains(payload.Name, "Time") {
		if resp.StatusCode == 403 || resp.StatusCode == 406 || resp.StatusCode == 429 { return }
		if duration >= localThreshold {
			...
			if errV2 == nil {
				...
				if time.Since(startV2).Seconds() >= localThreshold {
					isVulnerable = true
					detail = fmt.Sprintf("Confirmed Time-Based SQLi (Sleep: %.2fs).", duration)
				}
			}
		}
	}
	...
	if isVulnerable {
		...
		if !*isAlreadyVulnerable {
			*isAlreadyVulnerable = true
			...
			onFound(core.Finding{Type: "SQL Injection", Target: target, Detail: detail, Severity: "Critical"})
		}
		foundMu.Unlock()
	}
}
